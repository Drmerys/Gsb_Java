...
public class RapportModel {
    ...
    public RapportModel(Connection connect, int index){
    ...
    public void result(){
        ...
        try (Statement state = connect.createStatement(); ResultSet result = state.executeQuery(query)){
            ...
            setPraticien(resultArray.get(index+1).toString()+" "+resultArray.get(index+2).toString());
            setDate(resultArray.get(index+3).toString());
            setMotif(resultArray.get(index+4).toString());
            setBilan(resultArray.get(index+5).toString());
        }
        catch(Exception e){
            e.printStackTrace();
