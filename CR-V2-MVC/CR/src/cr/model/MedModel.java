...
public class MedModel{
    ...
    public MedModel(Connection connect, int index){
    ...
    public void result(){
        ...
        try (Statement state = connect.createStatement(); ResultSet result = state.executeQuery(query)){
            ...
            setCode(resultArray.get(index).toString());
            setNom(resultArray.get(index+1).toString());
            setFam(resultArray.get(index+2).toString());
            setCompo(resultArray.get(index+3).toString());
            setEffets(resultArray.get(index+4).toString());
            setContrindic(resultArray.get(index+5).toString());
            setPrix(resultArray.get(index+6).toString());
        }
        catch(Exception e){
            e.printStackTrace();
